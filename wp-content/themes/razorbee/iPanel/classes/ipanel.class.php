...
if (!class_exists('IPANEL')) {
    ...
    class IPANEL
    {
            ...
            if ((isset($_GET['page'])) && ('ipanel_' . $this->args['ID'] == $_GET['page'])) {
                ...
                wp_localize_script(
                    ...
                    array(
                        ...
                        'reset_confirm_message' => esc_html__("Are you sure you want to reset back to the defaults?", 'thebuilt'),
                        'confirm_message' => esc_html__("Are you sure?", 'thebuilt'),
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $output['message'] = $output['status'] === 'error' ? esc_html__('Error!', 'thebuilt') : esc_html__('Options Updated.', 'thebuilt');
            $output['test'] = ($data);
        ...
        function ajaxRequestsHandle()
        {
            ...
            if ($_nonce === false) {
                die(
                    json_encode(
                        array(
                        'status' => 'error',
                        'message' => esc_html__('Error!', 'thebuilt')
                        )
                    )
                );
            }
            if ($_POST['action2'] == 'save_settings') {
                ...
                if ($_POST['import_type'] == 'code') { // Check if import data is code
                    ...
                    die(
                        json_decode(
                            array(
                            'status' => 'error',
                            'message' => esc_html__("You've not specified any data.", 'thebuilt')
                            )
                        )
                    );
                }
                ...
                if ($add_option === true) { // Check if add data was successfuly
                    ...
                    $output['message'] = esc_html__('Data has been imported successfully.', 'thebuilt');
                } else { // and if not
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('An error occured while importing data.', 'thebuilt');
                }
                ...
                $output['message'] = $delete === false ? esc_html__('Error!', 'thebuilt') : esc_html__('Options Reset.', 'thebuilt');
                $this->saveAllOptions();
                ...
                if (@unlink(urldecode($_POST['file_path']))) {
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('The file permanently deleted.', 'thebuilt');
                } else {
                    $output['status']  = 'error';
                    $output['message'] = esc_html__('ERROR!', 'thebuilt');
                }
                echo json_encode($output);
                ...
                if (current_user_can('upload_files')) {
                    ...
                    if (array_key_exists('error', $movefile)) {
                        ...
                        if ($is_image) {
                            ...
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'thebuilt').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'thebuilt').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
        ...
        public function downloadExport()
        {
            if (isset($_GET['action']) && $_GET['action'] == 'ipanel-download-export-file') {
                ...
                if ($data === false) {
                    wp_die(esc_html__('Error in download the file.', 'thebuilt'));
                }
            ...
            if ( false === ( get_transient( 'theme_googleFonts'  ) ) || sizeof(get_transient( 'theme_googleFonts'  )) == 0 ) {
              ...
              $google_fonts_data = file_get_contents(realpath(dirname(__FILE__)).'/googlefonts.json'); 
                ...
                if($google_fonts_data === false) {
                  ...
                  @$fonts->kind = 'error';
                  @$fonts->items[0]->kind = 'error';
                  @$fonts->items[0]->family = 'ERROR: Google Fonts file not found!';
        ...
        public function menuPage()
        {
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'thebuilt'); ?></button>
                ...
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/?from=hdwp_cp"><?php echo esc_html__('More themes', 'thebuilt'); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset', 'thebuilt'); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'thebuilt').'</button>'; 
                    } 
                ?>
                </div>
                <div class="clr"></div>
            ...
            <div id="ipanel-preview-wrap" style="display:none;">
                ...
                <div class="imac ipanel-device" id="ipanel-preview-desktop">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="1010" height="573"><?php echo esc_html__("Your browser doesn't support iframes.", 'thebuilt'); ?></iframe>
                </div>
                
                <div class="ipad ipanel-device" id="ipanel-preview-tablet">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="767" height="1037"><?php echo esc_html__("Your browser doesn't support iframes.", 'thebuilt'); ?></iframe>
                </div>
                
                <div class="iphone ipanel-device" id="ipanel-preview-phone">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="642" height="1127"><?php echo esc_html__("Your browser doesn't support iframes.", 'thebuilt'); ?></iframe>
                </div>
        ...
        public function quickUpload($options = null, $Repeater = array())
        {
            ...
            if (!empty($options['field_options']['btn_text'])) {
                ...
                $btn_text = esc_html__("Choose File", 'thebuilt');
            }
            ...
            if (filter_var($value['url'], FILTER_VALIDATE_URL) !== false) {
                ...
                if ($is_image) {
                    ...
                    $input .= '    '.esc_html__('File URL', 'thebuilt').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'thebuilt').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
                    $input .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $value['path'] . '">'.esc_html__('Delete', 'thebuilt').'</a>' . "\n";
                    $input .= '    </div>' . "\n";
        ...
        public function repeater($options = null)
        {
            ...
            if (count($value)) { // If repeater has any item
                foreach ($value as $itemKey => $itemVal) {
                    ...
                    $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'thebuilt') . '<span class="item_number"> ' . ($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '<span class="item_number">' . $i + 1 . '</span>') . '</h2>';
                ...
                $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
                $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
                $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'thebuilt') . ' 1' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '1') . '</h2>';
                $inputs .= '<div class="ipanel-repeater-fields">';
            ...
            $inputs .= '<button class="ipanel-button ipanel-button-white ipanel-repeater-new-item">'.esc_html__("New Item", 'thebuilt').'</button>';
        ...
        {
            $button_text  = isset($options['field_options']['button_text']) ? $options['field_options']['button_text'] : esc_html__("Upload", 'thebuilt');
            $media_title  = isset($options['field_options']['media_title']) ? $options['field_options']['media_title'] : esc_html__("Choose File", 'thebuilt');
            $media_button = isset($options['field_options']['media_insert_button_text']) ? $options['field_options']['media_insert_button_text'] : esc_html__("Choose File", 'thebuilt');
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $option_name = $Repeater['id'] . '[' . $Repeater['pos'] . '][' . $options['id'] . ']';
            } else {
                $option_name = $options['id'];
        ...
        public function export($options = null)
        {
            ...
            if ($export_value === false) {
                $export_value = esc_html__("You need to save options first.", 'thebuilt');
            }
            $op = '';
            ...
            $op .= '<a href="' . esc_url($url) . '" class="ipanel-button ipanel-button-white ipanel-export-link-btn" target="_blank"> ' . esc_html__('Download File', 'thebuilt') . '</a>';
            $op .= '</div>';
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . esc_html__('Enter Code', 'thebuilt') . '</p><textarea class="ipanel-import-text"></textarea></label>';
            $op .= '<br/><br/><label class="ipanel-import-file-label ipanel-button ipanel-button-white"> ' . esc_html__('Import File', 'thebuilt') . ' <input type="file" name="ipanel-import-file" class="ipanel-import-file"/></label>';
            $op .= '<span class="ipanel-chosen-filename"></span>';
            $op .= '<br/><br/><br/><a class="ipanel-button ipanel-button-white ipanel-import-button">'.esc_html__('Import', 'thebuilt').'</a>';
            $op .= '</div>';
            return $this->_fieldSection($options, $op);
        }
